...
기존의 컴파일러는 참조하는 데이터 타입이 바뀔 때 에러가 발생하여 찾기가 어렵지만 Swift는 이를 인식하여 사용자에게 알려준다.

타입을 바꾸기위해서는 Type casting이 필요하다.

### 타입별칭

---

데이터 타입을 사용자가 지정하여 사용가능 하다. → Alias(별칭)

### Tuple, Array, Dictionary, Set

---

- 튜플: 다양한 데이터 타입을 하나로 저장가능하다
- 배열: 한가지의 데이터 타입을 저장할 수 있고, C언어 에서 배열의 크기는 제한적이지만 Swift에서는 var일 경우 가변적으로 배열의 길이를 조절할 수 있다.
- 딕셔너리: Key-Value로 데이터를 저장가능하고 Key는 고유한 값을 가져야 한다. 값이 존재하지 않을때는 nil을 리턴한다.
- 집합: 유일한 값들로만 구성된다. 우선순위가 중요하지 않을경우 사용한다.

### enum

---

비슷한 유형의 값들을 저장할때 사용한다. 사용자에게 제한된 보기를 제공하거나 연관된 값을 저장하고 싶을 때 사용한다.
...
****프로퍼티****는 클래스, 구조체 또는 열거형 등에 관련된 값을 뜻한다. ****메서드****는 특정 타입에 관련된 함수를 의미한다. 앞서 공부했던 변수나 상수, 함수등이 어떤 목적으로 쓰이느냐, 어떻게 쓰이느냐에 따라 용어가 바뀐다
...
- 연산 프로퍼티: 특정 연산을 실행한 결과값을 의미(클래스, 구조체, 열거형 등에 쓰임)
